...
// 解锁托盘
static BOOL cdr_unlockdoor(int fd)
{
...
// 打开托盘
static BOOL cdr_opentray(int fd)
{
#if 0	
	int ret;
...
// 关闭托盘
static BOOL cdr_closetray(int fd)
{
#if 0		
	int ret;
...
static BOOL cdr_havedisc(int fd)
{
...
// 获得光驱Buffer能力
static int cdr_buffcap(int fd, uint32_t *buffsize, uint32_t *bufffree)
{
	struct{
		unsigned int pad;
...
static int cdr_discexacttype(int fd, int *type)
{
...
// 获得光盘基本类型
static int cdr_discbasictype(int fd, int *type)
{
	unsigned char Profile[512];
	int tmp;
...
static int cdr_getdiscinfo(int fd, int *TrickCount, int *SessionCount, uint64_t *Capicity, int *DiskStatus)
{
	disc_info_t di;
	int CapicitySize = 0;
...
// 获得光驱能力
static int cdr_devcapablity(int fd, int *capablity)
{
	return 0;
}

static int cdr_loadmedia(int fd)
{
	INIT_CGC();
...
int cdr_startStopUnit(int fd, int isStop)
{
	INIT_CGC();
	wait_unit_ready(fd, 120);
...
// 读轨道数据
static int cdr_readtrack(int fd, int start, uint8_t *pbuffer, int size)
{
	INIT_CGC();
...
// 暂停数据写入
static int cdr_pausewrite(int fd)
{
	return 0;
}

// 恢复数据写入
static int cdr_resumewrite(int fd)
{
	return 0;
}
...
// 打印光驱profile
static int cdr_dpdevprofile(int fd)
{
...
//获取光盘支持的最大速度
static int cdr_getmaxspeed(int fd,int *MaxReadSpeed,int *MaxWriteSpeed)
{
...
static int cdr_setspeed(int fd,int ReadSpeed,int WriteSpeed,int DiscType)
{
	int Speed;
...
static int cdr_getdiscusedsize(int fd,int * usedsize)
{
...
//获取仓门状态
static int cdr_getdoorstate(int fd)
{
...
static struct CDR_CMD_T cdr_dvd = {
	.cdr_lockdoor     		= cdr_lockdoor,
	.cdr_unlockdoor   		= cdr_unlockdoor,
	.cdr_opentray     		= cdr_opentray,
	.cdr_closetray    		= cdr_closetray,
	.cdr_havedisc			= cdr_havedisc,
	.cdr_formatdisc   		= cdr_formatdisc,
	.cdr_buffcap      		= cdr_buffcap,
	.cdr_discbasictype     	= cdr_discbasictype,
	.cdr_discexacttype     	= cdr_discexacttype,
	.cdr_getdiscinfo  		= cdr_getdiscinfo,
	.cdr_devcapablity 		= cdr_devcapablity,
	.cdr_loadmedia    		= cdr_loadmedia,
	.cdr_readtrack    		= cdr_readtrack,
	.cdr_writetrack   		= cdr_writetrack,
	.cdr_flushtrack   		= cdr_flushtrack,
	.cdr_pausewrite   		= cdr_pausewrite,
	.cdr_resumewrite  		= cdr_resumewrite,
	.cdr_gettrackinfo 		= cdr_gettrackinfo,
	.cdr_closetrack   		= cdr_closetrack,
	.cdr_closesession 		= cdr_closesession,
	.cdr_dpdevprofile 		= cdr_dpdevprofile,
	.cdr_getdevinfo   		= cdr_getdevinfo,
	.cdr_getmaxspeed  		= cdr_getmaxspeed,
	.cdr_setspeed  	  		= cdr_setspeed,
