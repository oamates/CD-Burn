...
//视频用这个
int LvDVDRecUdf_encode_utf8(char *out, char *hdr, char *in, int outlen)
{
	int inlen = strlen(in);
	int utf_cnt, len = 1, i;
...
int decode_string_utf16(char *in, char *out, int inlen)
{
	int i;
	for( i=1;i<inlen;i++)
	{
...
static int encode_string(struct udf_disc *disc, char *out, char *hdr, char *in, int outlen)
{
	int i;
...
//实现字符串的unicode编码,unicode使用2字节编码,该编码函数
static int encode_unicode(uint8_t *instr,uint8_t * outstr,int inlen)
{	
	//在unicode码表中定位:	 xxyy内码，  offset = (xx - 0xB0) * 94 + (yy - 0xA1);
	int i,j = 0, offset, count = 0;
	outstr[0] = 16;
	for( i = 0 ; i< inlen ; i++)
	{
		if ( (instr[i] >= 0xB0 && instr[i] <= 0xF7) && instr[i+1] >= 0xA1 && instr[i+1] <= 0xFE)
		{
			offset = (instr[i] - 0xB0) * 94 + (instr[i+1] - 0xA1);
			outstr[j] = ( gb2ucs_lookup[offset] & 0xFF00 ) >> 8;
			outstr[j+1] = ( gb2ucs_lookup[offset] & 0x00FF ) ;
			i++;
		}
		else
		{
			outstr[j] = 0;
			outstr[j+1] = (uint8_t)instr[i];
		}
		j+= 2;
		count += 2;
	}
...
{
	int count = encode_unicode(instr,&outstr[1],inlen);
	outstr[0] = 16;
	//outstr[outMaxlen - 1] = count + 1;
	return count;
}
