...
struct UDFIdentSuffix   //udf标识后缀
{
	uint16_t	UDFRevision;  //udf版本
	uint8_t		OSClass;      //操作系统分类
	uint8_t		OSIdentifier;  //操作系统标识
	uint8_t		reserved[4];   //保留
} __attribute__ ((packed));

struct impIdentSuffix        	//实施标识后缀
{     
	uint8_t		OSClass;     	//操作系统分类
	uint8_t		OSIdentifier;   //操作系统标识
	uint8_t		reserved[6];
...
struct logicalVolIntegrityDescImpUse  //逻辑卷标完整性实施使用,LVID_IU
{
	regid		impIdent;
	uint32_t	numFiles;
	uint32_t	numDirs;
