...
namespace Core2ReadFunction
{
							   ...
							   eMainChannelData MCD,eSubChannelData SCD,eC2ErrorInfo ErrorInfo)
	{
		...
		if (ulBlockCount > 0xFFFFFF)
			GetLogDlg()->print_line(_T("  Warning: Requested block count to large, trunkated the number of block requested to read."));
    ...
    CLogDlg * CReadFunction::GetLogDlg() const 
    { 
        return m_pLogDlg;
    }
    void CReadFunction::SetLogDlg(CLogDlg * val) 
    { 
        m_pLogDlg = val; 
    }

	CReadUserData::CReadUserData(ckmmc::Device &Device,ckcore::OutStream *pOutStream,CLogDlg *pLogDlg) :
		CReadFunction(Device),m_pOutStream(pOutStream)
	{
		...
		CCore2Info Core2Info(m_pLogDlg);
		unsigned long ulBlockAddress = 0;

		if (!Core2Info.ReadCapacity(Device,ulBlockAddress,m_ulFrameSize))
		{
			m_ulFrameSize = 2048;
			GetLogDlg()->print_line(_T("  Error: Unable to obtain disc block information, impossible to continue."));
		}

		GetLogDlg()->print_line(_T("  Block address: %u, block length: %u."),ulBlockAddress,m_ulFrameSize);
	}
    ...
    CLogDlg * CReadUserData::GetLogDlg() const 
    { 
        return m_pLogDlg;
    }
    void CReadUserData::SetLogDlg(CLogDlg * val) 
    { 
        m_pLogDlg = val; 
    }
...
CCore2Read::CCore2Read(CCore2 *pCore2)
{
    m_pCore2=pCore2;
}
CCore2Read::~CCore2Read()
								...
								unsigned char *pBuffer,unsigned long ulAddress)
{
	...
	for (unsigned int i = 0; i < CORE2_READ_RETRYCOUNT; i++)
	{
		...
		GetCore2()->WaitForUnit(Device,pProgress);
						  ...
						  unsigned long ulNumBlocks,bool bIgnoreErr)
{
	...
	{
		GetLogDlg()->print_line(_T("  Error: The selected device does not support this kind of operation."));
		return false;
	}
	...
	{
		GetLogDlg()->print_line(_T("  Error: No disc inserted."));
		return false;
	}
	...
	for (unsigned long l = ulStartBlock; l < ulEndBlock; l += ulReadCount)
	{
		...
		{
			GetLogDlg()->print_line(_T("  Warning: Failed to read sector range %u-%u"),
				l,l + ulReadCount);
			if (pProgress != NULL)
			...
			for (unsigned int j = 0; j < ulReadCount; j++)
			{
				...
				{
					GetLogDlg()->print_line(_T("    Retry on sector %u failed."),l+j);
		...
		{
			GetLogDlg()->print_line(_T("  Error: Unable to process read data."));
...
CLogDlg * CCore2Read::GetLogDlg() const 
{ 
    return m_pLogDlg;
}
void CCore2Read::SetLogDlg(CLogDlg * val) 
{ 
    m_pLogDlg = val; 
}

CCore2 * CCore2Read::GetCore2() const { return m_pCore2; }
void CCore2Read::SetCore2(CCore2 * val) { m_pCore2 = val; }