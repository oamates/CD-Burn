		...
		switch (ucEvent[2] & 0x07)
		{
			...
			case 2:		// Power management.
				if ((uiDescriptor & 0xFF0000) != 0x01)
				{
					...
					if (ucStartCount == 10)
					{
						m_pLogDlg->print_line(_T("  Warning: Unable to start the drive, aborting after %d retries."),ucStartCount);
						return true;
					}

					m_pLogDlg->print_line(_T("  The drive is not in active state."));
					...
					m_pLogDlg->print_line(_T("   -> Started the disc to make it ready for access."));
					ucStartCount++;
				}
...
bool CCore2::StartStopUnit(ckmmc::Device &Device,eLoadMedia Action,bool bImmed)
{
	...
	if (ucResult != SCSISTAT_GOOD)
	{
			...
			if (ucSense[12] == 0x3A)
			{
				switch (ucSense[13])
				{
					...
					case 0x02:	// MEDIUM NOT PRESENT ?TRAY OPEN
						...
						{
							m_pLogDlg->print_line(_T("  Warning: Unable to start media, trying to manually load it."));
		...
		m_pLogDlg->print_line(_T("  Error: Could not start or stop the unit."));

		// Dump CDB.
		m_pLogDlg->print(_T("  CDB:\t"));
		for (unsigned int i = 0; i < 6; i++)
		{
			if (i == 0)
				m_pLogDlg->print(_T("0x%.2X"),ucCdb[i]);
			else
				m_pLogDlg->print(_T(",0x%.2X"),ucCdb[i]);
		}

		m_pLogDlg->print_line(_T(""));

		// Dump sense information.
		m_pLogDlg->print_line(_T("  Sense:\t0x%.2X,0x%.2X,0x%.2X,0x%.2X,0x%.2X,0x%.2X,0x%.2X,0x%.2X"),
			ucSense[0],ucSense[1],ucSense[2],ucSense[3],
			ucSense[4],ucSense[5],ucSense[6],ucSense[7]);

		m_pLogDlg->print_line(_T("  \t0x%.2X,0x%.2X,0x%.2X,0x%.2X,0x%.2X,0x%.2X,0x%.2X,0x%.2X"),
					   ...
					   unsigned int uiSpeed)
{
	...
	{
		m_pLogDlg->print_line(_T("CCore2::EraseDisc"));
		...
		m_pLogDlg->print_line(szParameters);
	}
	...
	if (!UpdateModePage5(Device,bSimulate))
		m_pLogDlg->print_line(_T("  Warning: Unable to update code page 0x05."));

// Disabled until these routines are concidered stable enough.
#if 0
	// Lock the media.
	if (!LockMedia(pDevice,true))
		m_pLogDlg->print_line(_T("  Warning: Unable to lock device media."));
#endif

	// Set write speed.
	if (!SetDiscSpeeds(Device,0xFFFF,(unsigned short)uiSpeed))
		m_pLogDlg->print_line(_T("  Warning: Unable to set disc erase speed."));
	...
	switch (iMethod)
	{
		...
		case ERASE_FORMAT_FULL:
			{
				CCore2Format Core2Format(m_pLogDlg);
		...
		case ERASE_BLANK_SESSION:
			{
				CCore2Blank Core2Blank(m_pLogDlg);
	...
	if (!LockMedia(Device,false))
		m_pLogDlg->print_line(_T("  Warning: Unable to unlock device media."));

	// Eject the media (if requested).
	if (bResult && bEject)
	{
		if (!StartStopUnit(Device,LOADMEDIA_EJECT,true))
			m_pLogDlg->print_line(_T("  Warning: Unable to eject media."));
	}
						   ...
						   unsigned char ucTrackNumber,bool bIgnoreErr,const TCHAR *szFilePath)
{
	...
	if (g_GlobalSettings.m_bLog)
	{
		m_pLogDlg->print_line(_T("CCore2::ReadTrack"));
	...
	CCore2Info Info(m_pLogDlg);
	CCore2Read Read(this);
	...
	m_pLogDlg->print_line(_T("  Maximum read speed: %d KiB/s."),uiMaxReadSpeed);

	if (!SetDiscSpeeds(Device,0xFFFF,0xFFFF))
		m_pLogDlg->print_line(_T("  Warning: Unable to set the device read speed."));
	...
	{
		m_pLogDlg->print_line(_T("  First and last disc track number: %d, %d."),
		...
		{
			m_pLogDlg->print_line(_T("  Error: The requested track number is invalid."));
			return false;
		}
	}
	else
	{
		m_pLogDlg->print_line(_T("  Error: Unable to read TOC information to validate selected track."));
		return false;
	}
	...
	{
		m_pLogDlg->print_line(_T("  Error: Unable to read track information."));
		return false;
	}
	...
	ulTrackSize -= 150;
	m_pLogDlg->print_line(_T("  Track span: %d-%d."),ulTrackAddr,ulTrackAddr + ulTrackSize);
	...
	m_pLogDlg->print_line(_T("  Track length: %02d:%02d:%02d"),ulMin,ulSec,ulFrame);
...
#ifdef DEBUG
	m_pLogDlg->print_line(_T("  Tracks (start, length):"));
	...
	for (it = Tracks.begin(); it != Tracks.end(); it++)
	{
		...
		if (Info.ReadTrackInformation(Device,CCore2Info::TIT_LBA,(*it).m_ulTrackAddr,&TrackInfo))
		{
			...
			m_pLogDlg->print_line(_T("    %d, %02d:%02d:%02d (%d)"),(*it).m_ulTrackAddr,ulMin,ulSec,ulFrame,TrackInfo.m_ulTrackSize);
		}
	}
#endif
	...
	if (!OutStream.open())
	{
		m_pLogDlg->print_line(_T("  Error: Unable to open the output file: \"%s\"."),szFilePath);
		return false;
	}
...
bool CCore2::ReadFullTOC(ckmmc::Device &Device,const TCHAR *szFileName)
{
	...
	{
		m_pLogDlg->print_line(_T("  Error: Unable to open file \"%s\" for writing."),szFileName);
		return false;
	}
	...
	{
		m_pLogDlg->print_line(_T("  Error: Unable to write to file \"%s\"."),szFileName);
		return false;
	}
						...
						std::map<tstring,tstring> *pFilePathMap)
{
	...
	switch (GetProjectSettings()->m_iFileSystem)
	{
		case FILESYSTEM_ISO9660:
			FileSysType = GetProjectSettings()->m_bJoliet ?
	...
	switch (GetProjectSettings()->m_iIsoLevel)
	{
		case 0:
	...
	FileSys.set_long_joliet_names(GetProjectSettings()->m_bJolietLongNames);
	FileSys.set_interchange_level(InterchangeLevel);
	FileSys.set_include_file_ver_info(!GetProjectSettings()->m_bOmitVerNum);
	FileSys.set_relax_max_dir_level(GetProjectSettings()->m_bDeepDirs);

	FileSys.set_volume_label(GetProjectSettings()->m_szLabel);
	FileSys.set_text_fields(GetProjectSettings()->m_szSystem,
		GetProjectSettings()->m_szVolumeSet,GetProjectSettings()->m_szPublisher,
		GetProjectSettings()->m_szPreparer);
	FileSys.set_file_fields(GetProjectSettings()->m_szCopyright,
		GetProjectSettings()->m_szAbstract,GetProjectSettings()->m_szBibliographic);

	std::list<CProjectBootImage *>::const_iterator itBootImage;
	for (itBootImage = GetProjectSettings()->m_BootImages.begin(); itBootImage !=
		GetProjectSettings()->m_BootImages.end(); itBootImage++)
	{
	...
	if (GetProjectSettings()->m_bMultiSession)
		ulSectorOffset = (unsigned long)GetProjectSettings()->m_uiNextWritableAddr;

	ckfilesystem::FileSystemWriter FileSysWriter(*m_pLogDlg,FileSys,bFailOnError);
						...
						std::map<tstring,tstring> *pFilePathMap)
{
	...
	if (!FileStream.open())
	{
		m_pLogDlg->print_line(_T("  Error: Unable to obtain file handle to \"%s\"."),szFullPath);
		return RESULT_FAIL;
	}
...
CLogDlg * CCore2::GetLogDlg() const 
{ 
    return m_pLogDlg;
}
void CCore2::SetLogDlg(CLogDlg * val) 
{ 
    m_pLogDlg = val; 
}

CProjectSettings * CCore2::GetProjectSettings() const 
{ 
    return m_pProjectSettings; 
}
void CCore2::SetProjectSettings(CProjectSettings * val)
{
    assert(val != NULL);
