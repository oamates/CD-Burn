...
CCore2Format::CCore2Format(CLogDlg *pLogDlg)
{
    m_pLogDlg=pLogDlg;
}
	...
	while (true)
	{
		...
		if (GetTickCount() > (ulPrevTime + 1000))
		{
			if (pProgress->cancelled())
			{
				...
				GetCore2()->CloseTrackSession(Device,0,0,true);
				return false;
			}
...
CLogDlg * CCore2Format::GetLogDlg() const 
{ 
    return m_pLogDlg;
}
void CCore2Format::SetLogDlg(CLogDlg * val) 
{ 
    m_pLogDlg = val; 
}

CCore2 * CCore2Format::GetCore2() const { return m_pCore2; }
void CCore2Format::SetCore2(CCore2 * val) { m_pCore2 = val; }

bool CCore2Format::FormatUnit(ckmmc::Device &Device,CAdvancedProgress *pProgress,bool bFull)
{
	m_pLogDlg->print_line(_T("CCore2Format::FormatUnit"));
	...
	m_pLogDlg->print_line(_T("  Current profile: 0x%.4X."),usProfile);
	...
	{
		m_pLogDlg->print_line(_T("  Error: Unsupported media."));
		return false;
	}
	...
	m_pLogDlg->print_line(_T("  Capacity list length: %d bytes."),ucCapListLen);

	if (ucCapListLen % 8 != 0 || ucCapListLen == 0)
	{
		m_pLogDlg->print_line(_T("  Error: Invalid capacity list length."));
		return false;
	}
	...
	{
		m_pLogDlg->print_line(_T("  Error: Unable to determine media capacity."));
		return false;
	}
	else
	{
		...
		m_pLogDlg->print_line(_T("  Disc capacity: %.2f GiB (%I64d bytes)."),
			((double)uiCapacity * 2048)/1073741824,(__int64)uiCapacity * 2048);

		if ((ucBuffer[8] & 0x03) == 0x01)	// Unformatted or blank media.
			m_pLogDlg->print_line(_T("  The disc media is unformatted or blank."));
		else if ((ucBuffer[8] & 0x03) == 0x02)	// Formatted media.
			m_pLogDlg->print_line(_T("  The disc media is formatted."));
	}
	...
	if (usProfile == PROFILE_DVDPLUSRW || usProfile == PROFILE_DVDPLUSRW_DL)
	{
		...
		if (!GetCore2()->CloseTrackSession(Device,0x00,0x00,true))
