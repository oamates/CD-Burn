...
CCore2Info::CCore2Info(CLogDlg *pLogDlg)
{
									  ...
									  unsigned long ulTrackAddr,CCore2TrackInfo *pTrackInfo)
{
	...
	if (usDataLength > (sizeof(ucBuffer) - 2))
		GetLogDlg()->print_line(_T("  Warning: CCore2::ReadTrackInformation received more track information than it could handle."));
...
{
	GetLogDlg()->print_line(_T("CCore2Info::GetTotalDiscCapacity"));
	...
	GetLogDlg()->print_line(_T("  Current profile: 0x%.4X."),usProfile);
	...
	GetLogDlg()->print_line(_T("  Capacity list length: %d bytes."),ucCapListLen);

	if (ucCapListLen % 8 != 0 || ucCapListLen == 0)
	{
		GetLogDlg()->print_line(_T("  Error: Invalid capacity list length."));
		return false;
	}
	...
	{
		GetLogDlg()->print_line(_T("  No media present."));
		return false;
	}
...
CLogDlg * CCore2Info::GetLogDlg() const 
{ 
    return m_pLogDlg;
}
void CCore2Info::SetLogDlg(CLogDlg * val) 
{ 
    m_pLogDlg = val; 
}