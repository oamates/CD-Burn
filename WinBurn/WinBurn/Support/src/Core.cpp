...
unsigned char CCore::GetBurnedPercentage() const
{
    return m_burnedPercentage;
}

void CCore::SetBurnedPercentage(unsigned char val)
{
    m_burnedPercentage = val;
}
...
bool CCore::SafeLaunch(tstring &CommandLine,bool bWaitForProcess)
{
    if (g_GlobalSettings.m_bLog)
        m_pLogDlg->print_line(_T("  Command line to run: %s"),CommandLine.c_str());
    ...
    if (CommandLine.length() > MAX_PATH - 1)
    {
            ...
            (g_WinVer.m_ulMajorVersion == MAJOR_WINXP && g_WinVer.m_ulMinorVersion < MINOR_WINXP))
        {
            ...
            if (g_GlobalSettings.m_bLog)
                m_pLogDlg->print_line(_T("  Warning: The command line is %d characters long. Trying to execute through shell."),CommandLine.length());
...
void CCore::event_output(const std::string &block)
{
    ...
    {
        m_pLogDlg->print(_T("   > "));
        m_pLogDlg->print_line(ckcore::string::ansi_to_auto<1024>(block.c_str()).c_str());
    }
    ...
    else if (m_iStatusMode == SMODE_PROGRESS)
    {
        ...
        *pBuffer++;
        ...
        // Update the status.
        m_pProgress->set_status(lngGetString(STATUS_WRITE),fSpeed, (int)m_TrackSize.size(), m_uiCurrentTrack + 1);

        m_pProgress->set_progress((unsigned char)(((double)(iWritten + m_uiProcessedSize)/m_uiTotalSize) * 100));
        m_pProgress->SetBuffer(iBuffer);

        SetBurnedPercentage((unsigned char)(((double)(iWritten + m_uiProcessedSize)/m_uiTotalSize) * 100));
...
void CCore::event_finished()
{
    ...
    if (g_GlobalSettings.m_bLog)
    {
        m_pLogDlg->print_line(_T("CCore::ProcessEnded"));
        m_pLogDlg->print_line(_T("  Process exited with code: %d."),uiExitCode);
    }
...
bool CCore::EjectDisc(ckmmc::Device &Device,bool bWaitForProcess)
{
    ...
    if (g_GlobalSettings.m_bLog)
    {
        m_pLogDlg->print_line(_T("CCore::EjectDisc"));
        m_pLogDlg->print_line(_T("  [%d,%d,%d] %s"),Device.address().bus_,
...
bool CCore::LoadDisc(ckmmc::Device &Device,bool bWaitForProcess)
{
    ...
    if (g_GlobalSettings.m_bLog)
    {
        m_pLogDlg->print_line(_T("CCore::LoadDisc"));
        m_pLogDlg->print_line(_T("  [%d,%d,%d] %s"),Device.address().bus_,
            Device.address().target_,Device.address().lun_,
            Device.name());
    }
                      ...
                      int iMode,bool bForce,bool bEject,bool bSimulate)
{
    ...
    if (g_GlobalSettings.m_bLog)
    {
        m_pLogDlg->print_line(_T("CCore::EraseDisc"));
        m_pLogDlg->print_line(_T("  [%d,%d,%d] %s"),Device.address().bus_,
            Device.address().target_,Device.address().lun_,
            Device.name());
        m_pLogDlg->print_line(_T("  Mode = %d, Force = %d, Eject = %d, Simulate = %d."),
    ...
    m_pLogDlg->print_line(L"Before SafeLaunch\n");
                       ...
                       bool bEject,bool bSimulate)
{
    ...
    if (g_GlobalSettings.m_bLog)
    {
        m_pLogDlg->print_line(_T("CCore::FixateDisc"));
        m_pLogDlg->print_line(_T("  [%d,%d,%d] %s"),Device.address().bus_,
            Device.address().target_,Device.address().lun_,
            Device.name());
        m_pLogDlg->print_line(_T("  Eject = %d, Simulate = %d."),
                      ...
                      const TCHAR *szFileName,bool bWaitForProcess,bool bCloneMode)
{
    ...
    if (g_GlobalSettings.m_bLog)
    {
        m_pLogDlg->print_line(_T("CCore::BurnImage"));
        m_pLogDlg->print_line(_T("  [%d,%d,%d] %s"),Device.address().bus_,
            Device.address().target_,Device.address().lun_,
            Device.name());
        m_pLogDlg->print_line(_T("  File = %s."),szFileName);
        m_pLogDlg->print_line(_T("  Eject = %d, Simulate = %d, BUP = %d, Pad tracks = %d, Close = %d, Overburn = %d, Swab = %d, Ignore size = %d, Immed = %d, Audio master = %d, Forcespeed = %d, VariRec (enabled) = %d, VariRec (value) = %d, Clone = %d."),
            (int)GetBurnImageSettings()->m_bEject,
            (int)GetBurnImageSettings()->m_bSimulate,
            (int)GetBurnImageSettings()->m_bBUP,
            (int)GetBurnImageSettings()->m_bPadTracks,
            (int)GetBurnImageSettings()->m_bFixate,
            (int)m_burnAdvancedSettings.m_bOverburn,
            (int)m_burnAdvancedSettings.m_bSwab,
            (int)m_burnAdvancedSettings.m_bIgnoreSize,
            (int)m_burnAdvancedSettings.m_bImmed,
            (int)m_burnAdvancedSettings.m_bAudioMaster,
            (int)m_burnAdvancedSettings.m_bForceSpeed,
            (int)m_burnAdvancedSettings.m_bVariRec,
            m_burnAdvancedSettings.m_iVariRec,
            (int)bCloneMode);
    }
    ...
    m_lNumCopies = GetBurnImageSettings()->m_lNumCopies;
    ...
    if (Device.support(ckmmc::Device::ckDEVICE_TEST_WRITE) &&
        GetBurnImageSettings()->m_bSimulate)
    {
        CommandLine += _T(" -dummy");
    }

    // Write method.
    switch (GetBurnImageSettings()->m_iWriteMethod)
    {
    ...
    if (Device.support(ckmmc::Device::ckDEVICE_BUP))
    {
        ...
        if (GetBurnImageSettings()->m_bBUP)
    ...
    {
        if (m_burnAdvancedSettings.m_bAudioMaster)
        {
    ...
    {
        if (!m_burnAdvancedSettings.m_bForceSpeed)
        {
    ...
    {
        if (m_burnAdvancedSettings.m_bVariRec)
        {
            TCHAR szVariRec[32];
            lsprintf(szVariRec,_T("varirec=%d,"),m_burnAdvancedSettings.m_iVariRec);
    ...
    // Pad tracks.
    if (GetBurnImageSettings()->m_bPadTracks)
        CommandLine += _T(" -pad");

    // Fixate.
    if (!GetBurnImageSettings()->m_bFixate)
        CommandLine += _T(" -nofix");

    // Overburning.
    if (m_burnAdvancedSettings.m_bOverburn)
        CommandLine += _T(" -overburn");

    // Swap audio byte order. FIXME: Should possibly check for support before selecting.
    if (m_burnAdvancedSettings.m_bSwab)
        CommandLine += _T(" -swab");

    // Ignore size.
    if (m_burnAdvancedSettings.m_bIgnoreSize)
        CommandLine += _T(" -ignsize");

    // SCSI IMMED flag.
    if (m_burnAdvancedSettings.m_bImmed)
        CommandLine += _T(" -immed");

    // Speed.
    if (GetBurnImageSettings()->m_uiWriteSpeed != -1)
    {
        lsprintf(szBuffer,_T(" speed=%d"),GetBurnImageSettings()->m_uiWriteSpeed);
        CommandLine += szBuffer;
    }
...
{// XXXX  XXXX
    ...
    if (g_GlobalSettings.m_bLog)
    {
        m_pLogDlg->print_line(_T("CCore::BurnTracks"));
        m_pLogDlg->print_line(_T("  [%d,%d,%d] %s"),Device.address().bus_,
            Device.address().target_,Device.address().lun_,
            Device.name());

        if (szDataTrack != NULL)
            m_pLogDlg->print_line(_T("  File = %s."),szDataTrack);

        m_pLogDlg->print_line(_T("  Eject = %d, Simulate = %d, BUP = %d, Pad tracks = %d, Close = %d, Method = %d, Overburn = %d, Swab = %d, Ignore size = %d, Immed = %d, Audio master = %d, Forcespeed = %d, VariRec (enabled) = %d, VariRec (value) = %d, Mode = %d."),
            (int)GetBurnImageSettings()->m_bEject,
            (int)GetBurnImageSettings()->m_bSimulate,
            (int)GetBurnImageSettings()->m_bBUP,
            (int)GetBurnImageSettings()->m_bPadTracks,
            (int)GetBurnImageSettings()->m_bFixate,
            GetBurnImageSettings()->m_iWriteMethod,
            (int)m_burnAdvancedSettings.m_bOverburn,
            (int)m_burnAdvancedSettings.m_bSwab,
            (int)m_burnAdvancedSettings.m_bIgnoreSize,
            (int)m_burnAdvancedSettings.m_bImmed,
    ...
    // File name.
    TCHAR szCygwinFileName[MAX_PATH + 16]={0};
                          ...
                          int iMode,bool bWaitForProcess)
{
    ...
    if (g_GlobalSettings.m_bLog)
    {
        m_pLogDlg->print_line(_T("CCore::ReadDataTrack"));
        m_pLogDlg->print_line(_T("  File = %s."),szFileName);
        m_pLogDlg->print_line(_T("  [%d,%d,%d] %s"),Device.address().bus_,
            Device.address().target_,Device.address().lun_,
            Device.name());
        m_pLogDlg->print_line(_T("  Start = %d, End = %d."),ulStartSector,ulEndSector);
    }
                           ...
                           int iMode,bool bWaitForProcess)
{
    ...
    if (g_GlobalSettings.m_bLog)
    {
        m_pLogDlg->print_line(_T("CCore::ReadAudioTrack"));
        m_pLogDlg->print_line(_T("  File = %s."),szFileName);
        m_pLogDlg->print_line(_T("  [%d,%d,%d] %s"),Device.address().bus_,
            Device.address().target_,Device.address().lun_,
            Device.name());
                      ...
                      unsigned long ulEndSector,int iMode,bool bWaitForProcess)
{
    ...
    if (g_GlobalSettings.m_bLog)
    {
        m_pLogDlg->print_line(_T("CCore::ScanTrack"));
        m_pLogDlg->print_line(_T("  [%d,%d,%d] %s"),Device.address().bus_,
            Device.address().target_,Device.address().lun_,
            Device.name());
        m_pLogDlg->print_line(_T("  Start = %d, End = %d."),ulStartSector,ulEndSector);
...
g_BurnImageSettings, m_burnAdvancedSettings and g_ReadSettings (m_bIgnoreErr
only) objects.
*/
                     ...
                     CAdvancedProgress *pProgress)
{
    ...
    if (g_GlobalSettings.m_bLog)
    {
        m_pLogDlg->print_line(_T("CCore::CopyDisc"));
        m_pLogDlg->print_line(_T("  Source: [%d,%d,%d] %s"),SrcDevice.address().bus_,
            SrcDevice.address().target_,SrcDevice.address().lun_,
            SrcDevice.name());
        m_pLogDlg->print_line(_T("  Target: [%d,%d,%d] %s"),DstDevice.address().bus_,
            DstDevice.address().target_,DstDevice.address().lun_,
            DstDevice.name());
        m_pLogDlg->print_line(_T("  Eject = %d, Simulate = %d, BUP = %d, Pad tracks = %d, Close = %d, Overburn = %d, Swab = %d, Ignore size = %d, Immed = %d, Audio master = %d, Forcespeed = %d, VariRec (enabled) = %d, VariRec (value) = %d, Ignore read errors = %d."),
            (int)GetBurnImageSettings()->m_bEject,
            (int)GetBurnImageSettings()->m_bSimulate,
            (int)GetBurnImageSettings()->m_bBUP,
            (int)GetBurnImageSettings()->m_bPadTracks,
            ...
            (int)m_burnAdvancedSettings.m_bVariRec,
            m_burnAdvancedSettings.m_iVariRec,
            (int)m_readDiscSettings.m_bIgnoreErr);
    }
    ...
    // We need to specify the total size that we should record.
    m_uiProcessedSize = 0;
    m_uiTotalSize = m_copyDiscSettings.m_uiSourceSize / (1024 * 1024);		// MB.
    m_TrackSize.push_back(m_uiTotalSize);
    ...
    if (GetBurnImageSettings()->m_uiWriteSpeed != -1)
    {
        sprintf(szBuffer," speed=%d",GetBurnImageSettings()->m_uiWriteSpeed);
        CommandLine += szBuffer;
    }

    // Ignore read errors.
    if (m_readDiscSettings.m_bIgnoreErr)
    ...
    if (DstDevice.support(ckmmc::Device::ckDEVICE_VARIREC))
    {
        ...
        {
            char szVariRec[16];
            sprintf(szVariRec,"varirec=%d,",m_burnAdvancedSettings.m_iVariRec);
                     ...
                     int iMode,bool bWaitForProcess)
{
    ...
    if (g_GlobalSettings.m_bLog)
    {
        m_pLogDlg->print_line(_T("CCore::ReadDisc"));
        m_pLogDlg->print_line(_T("  File = %s."),szFileName);
        m_pLogDlg->print_line(_T("  [%d,%d,%d] %s"),Device.address().bus_,
            Device.address().target_,Device.address().lun_,
            Device.name());
        m_pLogDlg->print_line(_T("  Ignore read errors = %d, Clone = %d, Speed = %d."),
            (int)m_readDiscSettings.m_bIgnoreErr,
            (int)m_readDiscSettings.m_bClone,
            m_readDiscSettings.m_iReadSpeed);
    }
    ...
    // Clone.
    if (m_readDiscSettings.m_bClone)
        CommandLine += _T(" -clone");

    // Speed.
    if (m_readDiscSettings.m_iReadSpeed != -1)
    {
        TCHAR szBuffer[64];
        lsprintf(szBuffer,_T(" speed=%d"),m_readDiscSettings.m_iReadSpeed);
    }
...
DWORD WINAPI CCore::CreateCompImageThread(LPVOID lpThreadParameter)
{
    ...
    CCore *pCore=(CCore *)(((CCompImageParams *)lpThreadParameter)->pCustom);

    pCore->GetCore2()->CreateImage(pParams->m_Process,pParams->m_Files,
...
DWORD WINAPI CCore::NextCopyThread(LPVOID lpThreadParameter)
{
    ...
    lsprintf(szBuffer,lngGetString(INFO_CREATECOPY),
        pCore->GetBurnImageSettings()->m_lNumCopies - pCore->m_lNumCopies + 1,
        pCore->GetBurnImageSettings()->m_lNumCopies);
                            ...
                            int iDataMode,unsigned __int64 uiDataBytes,int iMode)
{
    ...
    if (g_GlobalSettings.m_bLog)
    {
        m_pLogDlg->print_line(_T("CCore::BurnCompilation"));
        m_pLogDlg->print_line(_T("  [%d,%d,%d] %s"),Device.address().bus_,
            Device.address().target_,Device.address().lun_,
            Device.name());
        m_pLogDlg->print_line(_T("  Eject = %d, Simulate = %d, BUP = %d, Pad tracks = %d, Close = %d, Overburn = %d, Swab = %d, Ignore size = %d, Immed = %d, Audio master = %d, Forcespeed = %d, VariRec (enabled) = %d, VariRec (value) = %d, Data bytes %I64d."),
            (int)GetBurnImageSettings()->m_bEject,
            (int)GetBurnImageSettings()->m_bSimulate,
            (int)GetBurnImageSettings()->m_bBUP,
            (int)GetBurnImageSettings()->m_bPadTracks,
    ...
    CCompImageParams CompImageParams(*this,Progress,Files,this);
...
CLogDlg * CCore::GetLogDlg() const 
{ 
    return m_pLogDlg;
}
void CCore::SetLogDlg(CLogDlg * val) 
{ 
    m_pLogDlg = val; 
}

CCore2 * CCore::GetCore2() const { return m_pCore2; }
void CCore::SetCore2(CCore2 * val) { m_pCore2 = val; }

CBurnImageSettings * CCore::GetBurnImageSettings() const
{
    return m_pBurnImageSettings;
}
void CCore::SetBurnImageSettings(CBurnImageSettings * val) 
{ 
