 ...
 * 104 try n times, give up reconnect
 * 201 lost packet
 * @nStatusValue
 * 0 OK
 * -1 ERROR
...
{
    CHNSYS_UINT         nTotalPacketCount;
    CHNSYS_UINT         nTotalLostPacketCount;
...
CHNSYS_INT RTSP_CLIENT_GetStatistics(RTSP_CLIENT_HANDLE hRTSPClient, RTSP_CLIENT_STATISTICSDATA *pStatistics);
...
CHNSYS_INT RTSP_CLIENT_GetStreamType(RTSP_CLIENT_HANDLE hRTSPClient, RTSP_CLIENT_HANDLE nStreamNo);
...
VOID RTSP_RTSP_FreeVideoProperty(RTSP_RTSP_VIDEOPROPERTY hVideoProperty);

CHNSYS_INT RTSP_CLIENT_GetAudioProperty(RTSP_CLIENT_HANDLE hRTSPClient, CHNSYS_INT nStreamNo, RTSP_RTSP_AUDIOPROPERTY *pAACProperty);
...
RTSP_SERVER_HANDLE RTSP_SERVER_CreateInstance(CHNSYS_INT nType);
...
typedef int (*RTSP_SERVER_StatusCallBackFunc)(RTSP_CLIENT_STATUSCBDATA scbd);
...
// for advance use;
VOID RTSP_SERVER_SetSDP(RTSP_SERVER_HANDLE hRTSPServer, CHNSYS_CHAR *sSDP, CHNSYS_INT nSDP);
