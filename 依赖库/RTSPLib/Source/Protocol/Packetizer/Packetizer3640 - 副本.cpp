...
int Packetizer3640::ProcessFrame(char *pData, int nData)
{
    ...
    if (GetFrameTimestamp() > m_nLastTimestamp)
    {
        ...
        char szFakeAudioData[10240];
        memset(szFakeAudioData, 0, 10240);
        while (nMoreSend > 0 && nMoreSend < 200)
        {
            ...
            memcpy(sRTPPacket+16, szFakeAudioData, nData);
            SendData(1, sRTPPacket, 16+nData);
        }
#endif
    }

    if (m_nLastFakeTime == 0)
    {
        m_nLastFakeTime = ZOS::milliseconds();
        m_nFakeCount = 0;
    }
    if (m_nFakeCount == 0 && (int)(ZOS::milliseconds()-m_nLastFakeTime) > 10000)
    {
        LOG_DEBUG(("----in---\r\n"));
        m_nFakeCount = 1;
        //m_nLastFakeTime = ZOS::milliseconds();
