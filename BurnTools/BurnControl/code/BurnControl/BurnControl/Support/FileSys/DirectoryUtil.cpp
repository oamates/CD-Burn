...
#ifdef WIN32
static CStringW EnsureSlashEnd(CStringW strDirPath);
#endif
...
bool DirectoryUtil::CreateDir(const char *szDirPath)
{
    if (
        (szDirPath != NULL)
        && (strlen(szDirPath) < static_cast<size_t>(msc_nMaxFilePath))
        )
    {
...
UINT64 DirectoryUtil::GetDirSize(const char *szDirPath)
{
    ...
    for (size_t i = 0; i < vectFileList.size(); i ++)
    {
        ...
        //}
        struct _stat64 thestat;
        CStringW strFileFullPath=CharsetConvertMFC::UTF8ToUTF16(vectFileList.at(i).c_str());
        if(::_wstat64(strFileFullPath.GetBuffer(), &thestat) >= 0)
        {
            nTotalSize += thestat.st_size;
        }
#else
...
static CStringW EnsureSlashEnd(CStringW strDirPath)
{
    if (strDirPath.GetLength() >= 2 && strDirPath.GetAt(1) == L':')
    {//windows path
        if (strDirPath.ReverseFind(L'\\') != (strDirPath.GetLength()-1))
        {
            return strDirPath + L"\\";
        }
    }
    else if (strDirPath.GetLength() >= 1 && strDirPath.GetAt(0) == L'/')
    {
        if (strDirPath.ReverseFind(L'/') != (strDirPath.GetLength()-1))
        {
            return strDirPath + L"/";
