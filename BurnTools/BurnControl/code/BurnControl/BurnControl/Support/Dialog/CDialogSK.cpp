...
{    
    if (!IsZoomed())//playback 正常状态允许拖动(最大化时不允许拖动)
    {
        if (m_bEasyMove)
...
//         pDC->StretchBlt(0, 0, rect.Width(), 100, &dc, 0, 0, m_dwWidth, 100, SRCCOPY);
        pDC->StretchBlt(0, 0, 200, 100, &dc, 0, 0, 200, 100, SRCCOPY);
        pDC->StretchBlt(200, 0, rect.Width()-200, 100, &dc, 200, 0, m_dwWidth-200, 100, SRCCOPY);
        pDC->StretchBlt(0,100,rect.Width(), rect.Height() - 130, &dc, 0, 100, m_dwWidth, m_dwHeight - 130,SRCCOPY);
        pDC->StretchBlt(0,rect.Height() - 30,rect.Width(), 30, &dc, 0, m_dwHeight - 30, m_dwWidth, 30,SRCCOPY);
    }
