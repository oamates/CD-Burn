...
#define BTNST_BS_TYPEMASK SS_TYPEMASK
...
CButtonST::CButtonST()
{
	...
	m_nTypeStyle = BTNST_BS_TYPEMASK;
...
void CButtonST::PreSubclassWindow() 
{
	...
	m_nTypeStyle = nBS & BTNST_BS_TYPEMASK;
	...
	ModifyStyle(BTNST_BS_TYPEMASK, BS_OWNERDRAW, SWP_FRAMECHANGED);
...
{
	UINT nNewType = (wParam & BTNST_BS_TYPEMASK);
	...
	return DefWindowProc(BM_SETSTYLE,
		(wParam & ~BTNST_BS_TYPEMASK) | BS_OWNERDRAW, lParam);
} // End of OnSetStyle
...
DWORD CButtonST::SetIcon(LPCTSTR lpszFileIconIn, LPCTSTR lpszFileIconOut)
{
    HICON		hIconIn			= NULL;
    HICON		hIconOut		= NULL;

    hIconIn = (HICON)::LoadImage(0, lpszFileIconIn, IMAGE_ICON, 0, 0, LR_LOADFROMFILE);
    hIconOut = (HICON)::LoadImage(0, lpszFileIconOut, IMAGE_ICON, 0, 0, LR_LOADFROMFILE);
...
DWORD CButtonST::SetBitmaps(LPCTSTR lpszFileBitmapIn, COLORREF crTransColorIn, HBITMAP hBitmapOut, COLORREF crTransColorOut)
{
    HBITMAP hBitmapIn = NULL;

    hBitmapIn = (HBITMAP)::LoadImage(0, lpszFileBitmapIn, IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
    if (hBitmapIn != NULL)
    {
        return SetBitmaps(hBitmapIn, crTransColorIn, hBitmapOut, crTransColorOut);
    }
